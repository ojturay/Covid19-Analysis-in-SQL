...
--total vaccination done per continent 
select continent, max(cast(total_vaccinations as int)) as Totalvaccinatedcontinent,
		max(cast(total_vaccinations as int))/
from CototalvavidVac
where continent is not  null
group by continent 
order by Totalvaccinatedcontinent desc 
